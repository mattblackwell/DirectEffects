...
#' @export
#' 
#'  @return Returns a list object containing the following
#' components:
#' \itemize{
#'   \item acde.sd: a vector of bootstrapped stanadard errors for each named coefficient for the 
...
boots_g = function(seqg, boots = 1000){
  require(R.utils)
  acde.boots <- ate.boots <- pt.boots <- rep(list(NA), times = boots)
  pb <- txtProgressBar(min = 0, max = boots, style = 3)
  for (b in 1:boots) {
    
    setTxtProgressBar(pb, b)
    draw = sample(1:nrow(seqg$model), replace = T)
    data.draw.first <- seqg$first_mod$model[draw,]
    data.draw.direct <- seqg$model[draw,]
    ...
    data.draw <- cbind(data.draw.direct[,shared],
                      data.draw.direct[,just.direct],
                       data.draw.first[,just.first])
    colnames(data.draw) = c(shared, just.direct, just.first)
    ate.mod <- lm(formula(seqg$formula, lhs = 1, rhs = 1), data = data.draw)
    boot.first <- lm(seqg$first_mod$call[[2]], data = data.draw)
    boot.direct <- sequential_g(seqg$formula, boot.first, data = data.draw)    
    acde.boots[[b]] <- coef(boot.direct)
    ate.boots[[b]] <- coef(ate.mod)
    pt.boots[[b]] <- coef(boot.first)
  }
  close(pb)

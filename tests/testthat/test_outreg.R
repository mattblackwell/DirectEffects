...
test_that(
  "logit predicted values", {
    ...
    expect_equivalent(pred_m_11, de_outreg_nocross$outreg_pred[[2L]][, "1_1"])
    ...
    expect_equivalent(pred_m_01, de_outreg_nocross$outreg_pred[[2L]][, "0_1"])
    ...
    expect_equivalent(pred_m_10, de_outreg_nocross$outreg_pred[[2L]][, "1_0"])
    ...
    expect_equivalent(pred_m_00, de_outreg_nocross$outreg_pred[[2L]][, "0_0"])
    ...
    expect_equivalent(pred_a_11, de_outreg_nocross$outreg_pred[[1L]][, "1_1"])

    or_a_01_mod <- lm(pred_m_01 ~ female + age_cat,
                      data = jc,
                      subset = treat == 0)
    pred_a_01 <- predict(or_a_01_mod, newdata = jc)
    expect_equivalent(pred_a_01, de_outreg_nocross$outreg_pred[[1L]][, "0_1"])

    or_a_10_mod <- lm(pred_m_10 ~ female + age_cat,
                      data = jc,
                      subset = treat == 1)
    pred_a_10 <- predict(or_a_10_mod, newdata = jc)
    expect_equivalent(pred_a_10, de_outreg_nocross$outreg_pred[[1L]][, "1_0"])

    or_a_00_mod <- lm(pred_m_00 ~ female + age_cat,
                      data = jc,
                      subset = treat == 0)
    pred_a_00 <- predict(or_a_00_mod, newdata = jc)
    expect_equivalent(pred_a_00, de_outreg_nocross$outreg_pred[[1L]][, "0_0"])

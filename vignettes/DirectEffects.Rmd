...
```{r, echo = TRUE, eval = F}
devtools::install_git("https://github.com/mattblackwell/DirectEffects.git")
library(DirectEffects)
```
...
## Second stage
Then, we implement `DirectEffects`. The main argument is a `formula` object as the first argument, regressing $Y$ on $X$ but mediated through $M$ (expressed as `yvar ~ xvars | mvars`).
...
Finally, we enter the two formula for each step into the sequential g-estimation function. 

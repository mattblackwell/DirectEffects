...
The CDE is useful for discriminating between  causal mechanisms, because the average total effect $\tau(a, a^\prime) \equiv E[(Y_i(a) - Y_i(a^\prime)]$ can be decomposed as the sum of the average CDE across observations (ACDE), the average natural indirect effect,  and a measure of how much the direct effect of $A$ depends on a particular $M = m$:
$$\tau(a, a^\prime) = ACDE (a, a^\prime, m = 0) +  ANIE (a, a^\prime) + E[M_i(a)[ CDE(a, a^\prime, m = 1) -  CDE(a, a^\prime, m = 0)]].$$

Thus, if the ACDE is non-zero, it is evidence that the effect of $A$ is not entirely explained/dominated by its mediated effect _via_ $M.$ We illustrate this with an empirical example.

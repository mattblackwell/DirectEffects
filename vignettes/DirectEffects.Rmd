...
# Setup
To show how to implement this procedure in R using the `DirectEffects` package, we first load the package and attach the example dataset on historical plow use and women's modern participation in politics:
...
Thus, if the _ACDE_ is non-zero, it is evidence that the effect of $A$ is not entirely explained/dominated by its mediated effect _via_ $M.$ We illustrate this with an emprical example.
...
* $X_i$ is the pre-treatment characteristics of the country, which are mostly geographic. (`tropical_climate`, `agricultura_suitability`, `large_animals`, `political_hierarchies`, `economic_complexity`, and `rugged`)

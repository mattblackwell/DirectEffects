...
telescopeMatch.result.0 <-  telescope_match(outcome = "exhealth30", treatment = "treat", mediator = "work2year2q", 
                                            ...
                                            s2.formula = tm_stage2, data=jobcorps_female, L=3, boot=F, verbose=T, ci=95)
```
...
Additional diagnostics can be conducted by calling the `balance()` function on the returned `tmatch` object to assess the change in pre-/post-matching covariate balance in both the first and second stages.

```{r}

## Assess mediator balance on selected pre- and post- treatment covariates
balance(telescopeMatch.result.0, vars = work2year2q ~ schobef + emplq4, data=jobcorps_female)

## Assess treatment balance on selected pre-treatment covariates
balance(telescopeMatch.result.0, vars = treat ~ trainyrbef + hhsize, data=jobcorps_female)
```

Calling the `plotDiag()` function will return a histogram of the number of times each unit is used as a match ($K/L$) in either the first (mediator) stage or the second (treatment) stage.

```{r}

# Number of times each unit is used as a match in the mediator (first) stage
plotDiag(telescopeMatch.result.0, stage="mediator")

# Number of times each unit is used as a match in the treatment (second) stage
plotDiag(telescopeMatch.result.0, stage="treatment")
